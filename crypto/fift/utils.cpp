...
td::Result<FiftOutput> mem_run_fift(std::string file, std::vector<std::string> args, std::string src_dir) {
  std::string source;
  if (src_dir.empty()) {
    auto data = td::read_file_str(file);
    if (data.is_ok()) {
      source = data.move_as_ok();
    }
  }
  if (source.empty()) {
    const std::array<std::string, 2> dirs = { fift_dir(src_dir), smartcont_dir(src_dir) };
    for (auto& dir : dirs) {
      auto data = td::read_file_str(dir + file);
      if (data.is_ok()) {
        source = data.move_as_ok();
        break;
      }
  ...
  if (source.empty()) {
    return td::Status::Error(PSTRING() << file << " is missing");
  }
  TRY_RESULT(source_lookup, create_source_lookup(std::move(source), true, true, true, true, true, true, true, src_dir));
  return mem_run_fift(std::move(source_lookup), std::move(args));
}
