...
class ValidatorElectionBidCreator : public td::actor::Actor {
 public:
  ...
  void signed_bid(td::BufferSlice signature) {
    ...
    auto R = fift::mem_run_fift("validator-elect-signed.fif",
        {wallet_, td::to_string(date_), td::to_string(frac),
...
class ValidatorPunishVoteCreator : public td::actor::Actor {
 public:
  ...
  void got_id(ton::PublicKey pubkey, size_t idx) {
    ...
    idx_ = idx;
    auto data = proposal_.as_slice().str();
    auto R = fift::mem_run_fift("complaint-vote-req.fif",
                                {td::to_string(idx_), td::to_string(election_id_), data},
                                dir_ + "/");
    if (R.is_error()) {
  ...
  void signed_vote(td::BufferSlice signature) {
    ...
    auto R = fift::mem_run_fift(
        ...
        {td::to_string(idx_), td::to_string(election_id_), data, key, sig}, dir_ + "/");
    if (R.is_error()) {
